  ...
  return (
    ...
    >
          ...
          {/* <button className=" bg-[#F4F4F4] p-1 rounded">
            <svg width="20" height="22" viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M13.3565 19.0194C13.3566 19.5839 13.1434 20.1276 12.7596 20.5415C12.3757 20.9555 11.8496 21.209 11.2867 21.2514L11.1189 21.257H8.8813C8.31679 21.2571 7.77307 21.0439 7.35914 20.6601C6.9452 20.2762 6.69166 19.7501 6.64932 19.1872L6.64372 19.0194H13.3565ZM10.0001 1.0674e-09C12.0307 -3.35021e-05 13.9819 0.788626 15.4421 2.19962C16.9024 3.61061 17.7575 5.53362 17.8271 7.563L17.8316 7.83151V12.0426L19.87 16.1195C19.959 16.2974 20.0035 16.4942 19.9998 16.693C19.9961 16.8919 19.9442 17.0869 19.8486 17.2613C19.753 17.4357 19.6166 17.5844 19.451 17.6946C19.2854 17.8047 19.0956 17.8731 18.8978 17.8939L18.7691 17.9006H1.23103C1.03207 17.9007 0.836061 17.8525 0.659801 17.7602C0.48354 17.6679 0.332284 17.5342 0.218994 17.3707C0.105703 17.2071 0.0337566 17.0185 0.00931879 16.8211C-0.015119 16.6236 0.00868087 16.4232 0.0786789 16.237L0.130143 16.1195L2.16857 12.0426V7.83151C2.16857 5.75447 2.99368 3.76249 4.46237 2.2938C5.93107 0.825103 7.92304 1.0674e-09 10.0001 1.0674e-09ZM10.0001 2.23757C8.55857 2.23766 7.17273 2.79422 6.13157 3.79119C5.09041 4.78816 4.47431 6.14858 4.41174 7.58874L4.40615 7.83151V12.0426C4.40616 12.3201 4.35457 12.5951 4.25399 12.8537L4.17009 13.0439L2.8611 15.663H17.1402L15.8312 13.0428C15.707 12.7948 15.63 12.5258 15.6041 12.2496L15.594 12.0426V7.83151C15.594 6.34791 15.0047 4.92507 13.9556 3.876C12.9065 2.82693 11.4837 2.23757 10.0001 2.23757Z"
                fill="black"
              />
            </svg>
          </button> */}
      ...
      <div className="flex w-full mt-3">
        {permissions?.view?.includes("dashboard") && (
          <button onClick={() => navigate("/dashboard")} className={" rounded-[20px_0_0_0] px-4 py-1 font-semibold transition-all duration-100 text-[15px] " + (location.pathname.includes("/dashboard") ? " bg-white" : " bg-[#C3CBFF]") + " "}>
            Dashboard 
          </button>
        )}
          ...
          <button id="dropMaster" className={" rounded-[20px_0_0_0] px-4 py-1 font-semibold  transition-all duration-100 text-[15px] z-[10]  relative" + (location.pathname.includes("/master/") ? " bg-white" : " bg-[#C3CBFF]")}>
            ...
            {dropDown === "master" && (
              <div className={"flex p-1 flex-col absolute shadow-md bg-white left-0 w-[150px] bottom-0 translate-y-[100%] justify-start shadow-[#00000034] text-sm font-normal "}>
                    ...
                    {typeMaster === "type" ? (
                      <ul className="list-item">
                        ...
                        {permissions?.view?.includes("customer type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/customer-type")}>
                            Customer Type
                          </li>
                        )}
                        {permissions?.view?.includes("account type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/account-type")}>
                            Account Type
                          </li>
                        )}
                        {permissions?.view?.includes("payment type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/payment-type")}>
                            Payment Type
                          </li>
                        )}
                        ...
                        {permissions?.view?.includes("document type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/document-type")}>
                            Document Type
                          </li>
                        )}
                        {permissions?.view?.includes("certification type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/certification-type")}>
                            Certification Type
                          </li>
                        )}
                        {permissions?.view?.includes("UOM type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/uom-type")}>
                            UOM Type
                          </li>
                        )}
                        {permissions?.view?.includes("tax type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/tax-type")}>
                            Tax Type
                          </li>
                        )}
                        ...
                        {permissions?.view?.includes("vendor type") && (
                          <li className="text-start ms-5" onClick={() => navigate("/master/type-master/vendor-type")}>
                            Vendor Type
                          </li>
                        )}
                ...
                {permissions?.view?.includes("customer master") && (
                  <button className="text-start" onClick={() => navigate("/master/customer-master")}>
                    Customer Master
                  </button>
                )}
                {permissions?.view?.includes("vendor master") && (
                  <button className="text-start" onClick={() => navigate("/master/vendor-master")}>
                    Vendor Master
                  </button>
                )}
        ...
        {getAllChildrens(createdModules[2]).some((child) => permissions?.view?.includes(child)) && (
          <button className={" rounded-[20px_0_0_0]  px-4 py-1 font-semibold transition-all duration-100 text-[15px] relative " + (location.pathname.includes("/inventory") ? " bg-white" : " bg-[#C3CBFF]")}>
            <p onClick={() => setDropDown(dropDown === "inventory" ? "" : "inventory")}>Inventory Management</p>
            {dropDown === "inventory" && (
              <div className={"flex p-1 z-[10] bg-white flex-col absolute shadow-md left-0 w-full bottom-0 translate-y-[100%] justify-start shadow-[#00000034] text-sm font-normal "}>
                {permissions?.view?.includes("purchase order") && (
                  <button className="text-start" onClick={() => navigate("/inventory/purchase-order")}>
                    Purchase Order
                  </button>
                )}
                ...
                {permissions?.view?.includes("stock check") && <button className="text-start"  onClick={() => navigate("/inventory/stock-check")}>Stock Check</button>}
          ...
          <button className={" rounded-[20px_0_0_0] px-4 py-1 font-semibold transition-all duration-100 text-[15px] relative " + (location.pathname?.includes("/production/") ? " bg-white" : " bg-[#C3CBFF]")}>
            ...
            {dropDown === "production" && (
              <div className={"flex bg-white p-1 flex-col absolute shadow-md left-0 w-full bottom-0 translate-y-[100%] justify-start shadow-[#00000034] text-sm font-normal "}>
                ...
                {permissions?.view?.includes("raw material utilization") && (
                  <button className="text-start" onClick={() => navigate("/production/raw-material-utilization")}>
        ...
        {/* {getAllChildrens(createdModules[5]).some((child) => permissions?.view?.includes(child)) && (
          <button className={" rounded-[20px_0_0_0] px-4 py-1 font-semibold transition-all duration-100 text-[15px] relative" + (location.pathname.includes("/supply-chain") ? " bg-white" : " bg-[#C3CBFF]")}>
            <p onClick={() => setDropDown(dropDown === "supply-chain" ? "" : "supply-chain")}>Supply Chain Management</p> {dropDown === "supply-chain" && <div className={"flex p-1 flex-col absolute shadow-md left-0 w-full bottom-0 translate-y-[100%] justify-start shadow-[#00000034] text-sm font-normal "}>{permissions?.view?.includes("finished goods outwards") && <button className="text-start" onClick={()=>navigate("/supply-chain/finished-goods-outward")}>Finished Goods Outward</button>}</div>}
          </button>
        )} */}
...
{getAllChildrens(createdModules[5]).some((child) => permissions?.view?.includes(child)) && (
          <button className={" rounded-[20px_0_0_0] px-4 py-1 font-semibold transition-all duration-100 text-[15px] relative " + (location.pathname?.includes("/supply-chain") ? " bg-white" : " bg-[#C3CBFF]")}>
            ...
            {dropDown === "supply-chain" && (
              <div className={"flex p-1 flex-col bg-white absolute shadow-md left-0 w-full bottom-0 translate-y-[100%] justify-start shadow-[#00000034] text-sm font-normal "}>
                {permissions?.view?.includes("finished goods outwards") && (
                  <button className="text-start" onClick={() => navigate("/supply-chain/finished-goods-outward")} >
                Finished Goods
                  </button>
                )}
                </div>
