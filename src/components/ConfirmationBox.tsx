...
function ConfirmationBox({ RejectFunction, ResolveFunction, message, ResolveButtonText, RejectButtonText }: Prop) {
    return (
        <div className='fixed h-screen w-screen bg-[#00000077] z-[999] top-0 left-0 flex justify-center items-center'>
            <div className='bg-white relative w-[400px] h-[200px] rounded-[20px_0_0_0] flex flex-col justify-center items-center gap-10'>
                ...
                <div className="flex gap-8">
                    <button className='border border-[#5970F5]  text-[#5970F5] px-5 py-1 rounded-md font-semibold' onClick={RejectFunction}>{RejectButtonText}</button>
                    <button className=' bg-[#5970F5]  text-white px-5 py-1 rounded-md font-semibold ' onClick={ResolveFunction}>{ResolveButtonText}</button>
                </div>
            </div>
        </div>
    )
