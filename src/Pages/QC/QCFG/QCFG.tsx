...
import { getAllProductFinishedGoods, getAllQCFG, getAllUserManagement, getAllVendorMaster, getType } from "../../../utils/redux/actions";
...
function QCFG() {
  ...
  const [dropDowns, setDropDown] = useState<{
    ...
    shippingMethods: any[];
    products: any[];
  }>({ vendor: [], account: [], discount: [], payment: [], document: [], vendorType: [], users: [], shippingMethods: [],uom:[],products:[] });
  ...
  useEffect(() => {
    dispatch(getAllQCFG()).then((res: any) => {
      setData(res.payload);
                ...
                <tbody className="overflow-auto text-xs text-center  text-[#5970F5] roboto-thin">
                  {filtered?.map((x: any, i: number) => (
                    <tr className="border relative">
                      <th>{i + 1}</th>
                      <th>{dropDowns?.products?.filter((y) => y?._id === x?.product)[0]?.productName}</th>
                      <th>{x?.productionQuantity}</th>
                      <th>{x?.rejected}</th>
                      <th>{+x?.productionQuantity - +x?.rejected}</th>
                      <th>{x?.doc}</th>
                      <th>{dropDowns?.uom?.filter((y) => y?._id === x?.uom)[0]?.value?.name}</th>
                      <th>{x?.batchNumber}</th>
                      <th className="relative ">
                        ...
                        <div className={"hidden hover:flex flex-col gap-[1px] absolute right-0 z-20 " + styles.option}>
                          <button onClick={() => navigate("/qc/qc-fg/check/" + x?._id)} className="bg-[#E0E4FF] rounded-md shadow-md shadow-[#00000040] gap-2 items-center text-black flex px-2 py-1">
