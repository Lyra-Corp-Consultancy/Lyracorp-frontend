...
import {
  activeAndDeactivePurchaseInward,
  getAllPurchaseInward,
...
function PurchaseInward() {
  ...
  const search = (val: string) => {
    ...
    const active = data.active.filter((x) => {
      if (
        vendor.includes(x?.vendor) ||
        x?.inwardDate?.toLowerCase().includes(lowerVal) ||
        x?.invoiceNumber?.toLowerCase().includes(lowerVal) ||
        x?.dcNumber?.toLowerCase().includes(lowerVal) ||
        x?.transporter?.toLowerCase().includes(lowerVal) ||
        x?.vehicleNumber?.toLowerCase().includes(lowerVal) ||
        x?.seq?.toLowerCase().includes(lowerVal)
      ) {
        return x;
      }
    });
    ...
    <div className="h-[83vh] w-screen">
      <div className="w-full px-5 h-[90%] pt-2">
        ...
        <div className="bg-[#F1F3FF] shadow-md mt-2 w-full p-4 rounded-lg h-full">
          ...
          <div className="bg-white rounded-lg w-full pt-3 h-[80%] shadow-md mt-4">
            <h2 className="roboto-bold ms-3 text-[20px] text-center">
              Purchase Inward List
            </h2>
            <div className="px-4 flex justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to={"/inventory/purchase-inward/add"}
      ...
      {confirmation && (
        <DeleteConfirmationBox
          message={
            active
              ? "Do you want to inactive this vendor?"
              : "Do you want to active this vendor?"
          }
          ...
          ResolveFunction={() => {
            if (active) {
              if (ActiveSelectUsers?.length > 0) {
                dispatch(
                  activeAndDeactivePurchaseInward(ActiveSelectUsers)
                ).then(() => {
                  dispatch(getAllPurchaseInward()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
              ...
              if (InactiveSelectUsers?.length > 0) {
                dispatch(
                  activeAndDeactivePurchaseInward(InactiveSelectUsers)
                ).then(() => {
                  dispatch(getAllPurchaseInward()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
