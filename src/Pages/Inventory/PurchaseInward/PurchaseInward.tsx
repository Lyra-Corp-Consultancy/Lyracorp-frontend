...
function PurchaseInward() {
  ...
  const search = (val: string) => {
    ...
    const active = data.active.filter((x) => {
      if (vendor.includes(x?.vendor) || x?.inwardDate?.toLowerCase().includes(lowerVal) || x?.invoiceNumber?.toLowerCase().includes(lowerVal) || x?.dcNumber?.toLowerCase().includes(lowerVal) || x?.transporter?.toLowerCase().includes(lowerVal) || x?.vehicleNumber?.toLowerCase().includes(lowerVal) || x?.seq?.toLowerCase().includes(lowerVal)) {
        return x;
      }
    });
