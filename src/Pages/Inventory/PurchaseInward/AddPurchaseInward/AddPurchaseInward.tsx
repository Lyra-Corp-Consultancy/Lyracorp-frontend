...
function AddPurchaseInward() {
  ...
  const handleSave = async () => {
    ...
    dispatch(addPurchaseInward({ ...val, lineOfBusiness })).then(() => {
      navigate(-1);
    });
  };
      ...
      <div className="bg-[#F1F3FF] shadow-md p-3 rounded-lg w-full">
        <div className="flex items-center mb-5 gap-3">
          ...
          <button
            type="button"
            onClick={() => {
              dispatch(getPurchaseOrdeBySerialNumber(serialNumber)).then(
                (res: any) => {
          ...
          <div className="grid grid-cols-4 items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                required
                value={data.dcNumber}
                onChange={(e) => setData({ ...data, dcNumber: e.target.value })}
                className="px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] h-[25px] w-[200px] rounded-md"
                type="text"
              />
            </div>
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                onChange={(e) =>
                  setData({ ...data, transporter: e.target.value })
                }
                className="px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] h-[25px] w-[200px] rounded-md"
                type="text"
              />
              ...
              >
                {dropDowns?.users[0]?.companyDetails[0]?.warehouse
                  ...
                  ?.filter((a: any) =>
                    a?.address
                      ?.toLowerCase()
                      ?.includes(searchValue?.warehouse?.toLowerCase() || "")
                  )
                  ?.map((x: any) => (
                    <li
                      onClick={() => {
                ...
                <tr className="text-center">
                    ...
                    >
                      {dropDowns?.uom
                        ?.filter((a) =>
                          a?.value?.name
                            ?.toLowerCase()
                            ?.includes(searchValue?.uom[i]?.toLowerCase() || "")
                        )
                        ?.map((x) => (
                          <li
                            onClick={() => {
                  ...
                  <td className="text-center border w-[100px] justify-center py-2 items-center ">
                    <div className="px-2 py-1 w-[90%]  bg-[#F6F4F4]  h-[25px] rounded-md">
                      <span>
                        {parseInt(x?.orderQuantity) -
                          parseInt(x?.recievedQuantity) >
                        0
                          ? parseInt(x?.orderQuantity) -
                            parseInt(x?.recievedQuantity)
                          : 0}
                  ...
                  <td className="text-center border justify-center py-2 items-center ">
                    <div className="px-2 py-1 w-[90%]  bg-[#F6F4F4]  h-[25px] rounded-md">
                      <span>
                        {parseFloat(x?.unitPrice) *
                          parseInt(x?.recievedQuantity) || 0}
                      </span>
                    </div>
                  </td>
                  <td className="text-center border justify-center py-2 items-center ">
                  ...
                  <td className="text-center border justify-center py-2 items-center ">
                    <div className="flex justify-center items-center">
                      <label>
                        ...
                        >
                          <path
                            d="M3.5 6V1.925L2.2 3.225L1.5 2.5L4 0L6.5 2.5L5.8 3.225L4.5 1.925V6H3.5ZM1 8C0.725 8 0.489667 7.90217 0.294 7.7065C0.0983332 7.51083 0.000333333 7.27533 0 7V5.5H1V7H7V5.5H8V7C8 7.275 7.90217 7.5105 7.7065 7.7065C7.51083 7.9025 7.27533 8.00033 7 8H1Z"
                            fill="#5970F5"
                          />
                        </svg>
