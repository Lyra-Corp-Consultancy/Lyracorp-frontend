...
import { getType } from '../../../../utils/redux/actions'
import { useNavigate} from "react-router-dom"

function AddCustomer() {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const [dropDowns, setDropDown] = useState<{ customer: any[], account: any[],discount:any[],payment:any[] }>({ customer: [], account: [],discount:[],payment:[] })
    ...
    useEffect(() => {
        ...
        res1.then((res: any) => {
            console.log(res.payload[0].customerType)
            setDropDown((prev) => {
                return {
                    ...prev,
        ...
        res2.then((res: any) => {
            console.log(res.payload[0].accountType)
            setDropDown((prev) => {
                return {
                    ...prev,
        ...
        dispatch(getType("discount")).then((res:any)=>{
            console.log(res.payload[0].discountType)
            setDropDown((prev) => {
                return {
                    ...prev,
        ...
        dispatch(getType("payment")).then((res:any)=>{
            console.log(res.payload[0].paymentType)
            setDropDown((prev) => {
                return {
                    ...prev,
            ...
            <div className='bg-[#F1F3FF] shadow-md p-3 rounded-lg w-full h-[90%]'>
                <div className='shadow-md bg-white px-4 h-full z-[0] relative rounded-lg pt-1 w-full'>
                    ...
                    <div className='grid grid-flow-col items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2'>
                        ...
                        <input type="text" className='px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md' />
                        <label>
                            Customer Type
                        </label>
                        <Select>
                            {dropDowns?.customer?.map((x) => (
                                <li className='px-3 hover:bg-slate-200 py-1 transition-all duration-100'>{x?.value}</li>
                            ))}
                        </Select>
                        <label>
                            Account Type
                    ...
                    <div className='grid grid-flow-col items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2'>
                        ...
                        <input type="email" className='px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md' />
                        <label>
                            Primary Number
                        </label>
                        <input type="number" className='px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md' />
                        <label>
                            Secondary
                        </label>
                        <input type="number" className='px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md' />
                    ...
                    <div className='flex flex-wrap gap-2 items-center justify-between roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2'>
                        ...
                        <div className='w-[50%] flex gap-3 items-center'>
                            ...
                            <textarea className='px-2 py-1 w-[77%] shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md' ></textarea>
                        </div>
                        <div className='w-[22%] flex gap-3 items-center'>
...
<p>preview {i+1}</p>
                    ...
                    <div className='w-full absolute bottom-4 justify-center items-center gap-3 flex mt-5'>
                        <button className='border rounded-md py-2 px-4 font-semibold border-[#5970F5] text-[#5970F5]'>Reset</button>
                        <button className='border rounded-md py-2 px-4 font-semibold border-[#5970F5] text-[#5970F5]' onClick={()=>navigate(-1)}>Cancel</button>
                        <button className=' rounded-md py-2 px-4 font-semibold bg-[#5970F5] text-white'>Save</button>
                    </div>
                </div>
            </div>
        </div>
