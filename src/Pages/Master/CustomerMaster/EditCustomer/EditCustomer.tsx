...
import { fileServer, phoneNumberDetails } from "../../../../utils/values/publicValues";
...
function EditCustomer() {
  ...
  const [searchValue, setSearchValue] = useState<{ customer?: string; account?: string; discount?: string; payment?: string; document?: string; countryCode: any }>({ customer: "", account: "", discount: "", payment: "", document: "", countryCode: "" });
  ...
  useEffect(() => {
    setSearchValue({...searchValue, customer: dropDowns?.customer?.filter((x) => x?._id === data?.customerType)[0]?.value,
                     ...
                     payment:dropDowns?.payment?.filter((x) => x?._id === data?.paymentTerms)[0]?.value,
                     document:dropDowns?.document?.filter((x) => x?._id === data?.bussinessDocument)[0]?.value,
                     countryCode:phoneNumberDetails.filter((x)=>x?.label===data?.country)[0]?.phone ? "+"+phoneNumberDetails.filter((x)=>x?.label===data?.country)[0]?.phone : ""
                    })
      ...
      <div className="bg-[#F1F3FF] shadow-md p-3 rounded-lg w-full min-h-[90%]">
        <form onSubmit={(e) => {
          ...
          <div className="grid grid-flow-col items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2">
            ...
            <input pattern="[A-Za-z0-9_]+" title="Users only can enter letter and number" required value={data.customerName} onChange={(e) => setData({ ...data, customerName: e.target.value })} type="text" className="px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md" />
            <label>Customer Type</label>
            <Select
              required
              pattern={dropDowns?.customer?.filter((a) => a?._id === data.customerType)[0] ? undefined : ""}
            ...
            >
              {dropDowns?.customer
                ?.filter((a) => a?.value?.toLowerCase()?.includes(searchValue?.customer?.toLowerCase() || ""))
                .map((x) => (
                  <li
                    onClick={() => {
            ...
            <Select
              required
              pattern={dropDowns?.account?.filter((a) => a?._id === data.accountType)[0] ? undefined : ""}
              title="Please Select values from drop down"
              onChange={(e) => {
            ...
            >
              {dropDowns?.account
                ?.filter((a) => a?.value?.toLowerCase()?.includes(searchValue?.account?.toLowerCase() || ""))
                .map((x) => (
                  <li
                    onClick={() => {
          ...
          <div className="grid grid-flow-col z-[100] items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2">
            ...
            <label className="px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] flex rounded-md">
              <Select
                ...
                onChange={(e) => {
                  setSearchValue({ ...searchValue, countryCode: e.target.value });
                }}
                className="border-none outline-none w-3/12"
              >
                {phoneNumberDetails
                  .filter((x) => ("+" + x.phone).includes(searchValue?.countryCode || ""))
                  .map((x) => (
                    <li
                      ...
                      onClick={() => {
                        ...
                        setSearchValue({ ...searchValue, countryCode: "+" + val.phone });
                        axios.post("https://countriesnow.space/api/v0.1/countries/states", { country: val.label }).then((res) => {
                          setPlaces({ ...places, state: res.data.data.states });
                          setSearch({ ...search, state: res.data.data.states });
                        });
                      }}
              ...
              <input min={phoneLength?.phoneLength || undefined} required className="ps-2 remove-spin-wheel w-9/12 border-none outline-none" value={data.primaryNumber} onChange={(e) => setData({ ...data, primaryNumber: e.target.value })} type="number" />
            </label>
            <label>Secondary Number</label>
            <input required min={phoneLength?.phoneLength || undefined} value={data.secondaryNumber} onChange={(e) => setData({ ...data, secondaryNumber: e.target.value })} type="number" className="px-2 py-1 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md remove-spin-wheel" />
          </div>
          <h1 className="roboto-medium mt-1">Address Details</h1>
          <div className={"flex flex-wrap gap-2 items-center  roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2 " + (data?.state && "justify-between")}>
            <div className="w-[22%] flex gap-3 items-center">
              <label>Country</label>
              <Select
              pattern={places?.country?.filter((a) => a?.country === data.country )[0] ? undefined : ""}
              title="Please Select values from drop down"
                required
                onChange={(e) => {
                ...
                {search?.country?.map((x) => (
                  <li
                    onClick={() => {
                      ...
                      const country = phoneNumberDetails.filter((y) => y.label.toLowerCase() === x?.country?.toLowerCase());
                      console.log(country);
                      setPhoneLength(country[0]);
                      setSearchValue({ ...searchValue, countryCode: "+" + country[0].phone });
                    }}
                    className="px-3 hover:bg-slate-200 py-1 transition-all duration-100"
                  >
                    {x?.country}
            ...
            {data?.country && (
              <div className="w-[22%] flex gap-3 items-center">
                <label>State</label>
                <Select
                pattern={places?.state?.filter((a) => a?.name === data.state )[0] ? undefined : ""}
              title="Please Select values from drop down"
                  required
                  onChange={(e) => {
            ...
            {data?.state && (
              <div className="w-[22%] z-10 flex gap-3 items-center">
                ...
                <Select
                pattern={places?.city?.filter((a) => a === data.city )[0] ? undefined : ""}
              title="Please Select values from drop down"
                  required
                  onChange={(e) => {
            ...
            {data?.state && (
              <>
                ...
                <div className="w-[22%] flex gap-3 items-center">
                  ...
                  <input required value={data.pincode} min={6} onChange={(e) => setData({ ...data, pincode: e.target.value })} type="number" className="px-2 py-1 remove-spin-wheel shadow-[0px_0px_4px_rgba(0,0,0,0.385)] rounded-md" />
                </div>
              </>
            )}
          </div>
          ...
          <div className="grid z-[98] grid-flow-col items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2">
            ...
            <Select
              pattern={dropDowns?.discount?.filter((a) => a?._id === data.discountType)[0] ? undefined : ""}
              title="Please Select values from drop down"
              required
              onChange={(e) => {
            ...
            >
              {dropDowns?.discount
                ?.filter((a) => a?.value?.toLowerCase()?.includes(searchValue?.discount?.toLowerCase() || ""))
                .map((x) => (
                  <li
                    onClick={() => {
            ...
            <Select
              pattern={dropDowns?.payment?.filter((a) => a?._id === data.paymentTerms)[0] ? undefined : ""}
              title="Please Select values from drop down"
              required
              onChange={(e) => {
          ...
          <div className="flex items-center gap-4 roboto-medium text-[13px] shadow-[0px_0px_4px_rgba(0,0,0,0.485)] w-full rounded-lg px-3 py-2">
            ...
            <Select
              pattern={dropDowns?.document?.filter((a) => a?._id === data.bussinessDocument)[0] ? undefined : ""}
              title="Please Select values from drop down"
              required
              onChange={(e) => {
            ...
            >
              {dropDowns?.document
                ?.filter((a) => a?.value?.toLowerCase()?.includes(searchValue?.document?.toLowerCase() || ""))
                .map((x) => (
                  <li
                    onClick={() => {
