...
import { activeAndDeactiveCustomerMaster, getAllCustomerMaster, getType } from "../../../utils/redux/actions";
...
function CustomerMaster() {
  ...
  const search = (val: string) => {
    ...
    const active = data.active.filter((x) => {
      if (x?.customerId?.toLowerCase()?.includes(lowerVal) || x?.customerName?.toLowerCase()?.includes(lowerVal) || x?.contactPerson?.toLowerCase()?.includes(lowerVal) || x?.primaryNumber?.toLowerCase()?.includes(lowerVal) || x?.address?.toLowerCase()?.includes(lowerVal) || x?.email?.toLowerCase()?.includes(lowerVal) || x?.purchaseResitriction?.toLowerCase()?.includes(lowerVal) || customer.includes(x?.customerType) || account.includes(x?.accountType) || discount.includes(x?.discountType) || payment.includes(x?.paymentTerms)) {
        return x;
      }
    });
    ...
    <div className="h-[83vh] w-screen">
        ...
        <div className="bg-[#F1F3FF] shadow-md mt-2 w-full p-4 rounded-lg h-full">
          <div className="flex gap-4">
            ...
            <button onClick={() => setActive(false)} className={"flex flex-col  rounded-md px-4 py-2 " + (!active ? "bg-[#5970F5] text-white" : "bg-[#C3CBFF] text-black ")}>
              <div className="flex items-center gap-3">
                ...
                <p className=" roboto-regular text-[15px]">Inactive Customers</p>
              </div>
          ...
          <div className="bg-white rounded-lg w-full pt-3 h-[80%] shadow-md mt-4">
            ...
            <div className="px-4 flex justify-between">
              ...
              {( permissions?.add?.includes("customer master")) &&   <Link to={"/master/customer-master/add-customer"} className="bg-[#5970F5] flex px-3 py-2 rounded-md text-white gap-2 items-center">
      ...
      {confirmation && (
        <DeleteConfirmationBox
          message={active ? "Do you want to inactive this customer?" : "Do you want to active this customer?"}
