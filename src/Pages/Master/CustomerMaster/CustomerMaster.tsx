...
import {
  activeAndDeactiveCustomerMaster,
...
function CustomerMaster() {
  ...
  const [filtered, setFiltered] = useState<{ active: any[]; deactive: any[] }>({
    active: [],
    deactive: [],
  });
  ...
  const exportCSV = () => {
    ...
    const dataForCvsExport = active ? data.active : data.deactive;

    for (let i = 0; i < dataForCvsExport?.length; i++) {
      const temp = dataForCvsExport[i];
      values[0][i] = i + 1;
      values[1][i] = temp?.customerId;
      values[2][i] = temp?.customerName;

      values[3][i] = dropDowns.customer.filter(
        (x) => x?._id === temp?.customerType
      )[0]?.value;
      values[4][i] = dropDowns.account.filter(
        (x) => x?._id === temp?.accountType
      )[0]?.value;
      values[5][i] = temp?.contactPerson;
      values[6][i] = temp?.primaryNumber;
      values[7][i] = temp?.address;
      values[8][i] = temp?.email;
      values[9][i] = temp?.purchaseResitriction;
      values[10][i] = dropDowns.discount.filter(
        (x) => x?._id === temp?.discountType
      )[0]?.value;
      values[11][i] = dropDowns.payment.filter(
        (x) => x?._id === temp?.paymentTerms
      )[0]?.value;
    }

  
    const csv = new CSV();
    csv.convertCSV(titles, values);
  };
  ...
  const search = (val: string) => {
    ...
    const active = data.active.filter((x) => {
      if (
        x?.customerId?.toLowerCase()?.includes(lowerVal) ||
        x?.customerName?.toLowerCase()?.includes(lowerVal) ||
        x?.contactPerson?.toLowerCase()?.includes(lowerVal) ||
        x?.primaryNumber?.toLowerCase()?.includes(lowerVal) ||
        x?.address?.toLowerCase()?.includes(lowerVal) ||
        x?.email?.toLowerCase()?.includes(lowerVal) ||
        x?.purchaseResitriction?.toLowerCase()?.includes(lowerVal) ||
        customer.includes(x?.customerType) ||
        account.includes(x?.accountType) ||
        discount.includes(x?.discountType) ||
        payment.includes(x?.paymentTerms)
      ) {
        return x;
      }
    });
  ...
  useEffect(() => {
    dispatch(getAllCustomerMaster()).then((res: any) => {
      setData(res.payload);
  ...
  return (
    <div className="h-[83vh] w-screen">
      <div className="w-full px-5 h-[90%] pt-2">
        <h1 className="text-xl roboto-bold">Customer Master</h1>
        <div className="bg-[#F1F3FF] shadow-md mt-2 w-full p-4 rounded-lg h-full">
          <div className="flex gap-4">
            <button
              onClick={() => setActive(true)}
              className={
                "flex flex-col  rounded-md px-4 py-2 " +
                (active
                  ? "bg-[#5970F5] text-white"
                  : "bg-[#C3CBFF] text-black ")
              }
            >
              <div className="flex items-center gap-3">
                ...
                <p className=" roboto-regular text-[15px]">Active Customers</p>
              </div>
          ...
          <div className="bg-white rounded-lg w-full pt-3 h-[80%] shadow-md mt-4">
            <h2 className="roboto-bold ms-3 text-[20px]">Customer List</h2>
            <div className="px-4 flex justify-between">
              <label
                className="flex px-3 w-2/5 rounded-md py-1 mt-2 shadow-[0px_0px_4px_rgba(0,0,0,0.385)] items-center gap-3"
                htmlFor=""
              >
                <svg
                  width="15"
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <path
                    d="M9.33333 6.22222L6.22222 3.11111V5.44444H0V7H6.22222V9.33333M14 10.8889V1.55556C14 1.143 13.8361 0.747335 13.5444 0.455612C13.2527 0.163888 12.857 0 12.4444 0H3.11111C2.69855 0 2.30289 0.163888 2.01117 0.455612C1.71944 0.747335 1.55556 1.143 1.55556 1.55556V3.88889H3.11111V1.55556H12.4444V10.8889H3.11111V8.55556H1.55556V10.8889C1.55556 11.3014 1.71944 11.6971 2.01117 11.9888C2.30289 12.2806 2.69855 12.4444 3.11111 12.4444H12.4444C12.857 12.4444 13.2527 12.2806 13.5444 11.9888C13.8361 11.6971 14 11.3014 14 10.8889Z"
                    fill="#5970F5"
                  />
                </svg>
                <svg
                ...
                {active ? (
                  <svg
                    className={
                      ActiveSelectUsers?.length > 0 ? "cursor-pointer" : ""
                    }
                    onClick={() => {
                  ...
                  <svg
                    className={
                      InactiveSelectUsers?.length > 0 ? "cursor-pointer" : ""
                    }
                    onClick={() => {
                      setConfirmation(true);
                    }}
                  ...
                  >
                    ...
                    />
                    <path
                      d="M18.2638 6.07031L20.0357 7.85493L11.7708 16.1958L7.37891 11.7659L9.15086 9.98128L11.7708 12.6139L18.2638 6.07031Z"
                      fill="#5970F5"
                      stroke="#5970F5"
                    />
                  </svg>
                ...
                {permissions?.add?.includes("customer master") && (
                  <Link
                    to={"/master/customer-master/add-customer"}
                    className="bg-[#5970F5] flex px-3 py-2 rounded-md text-white gap-2 items-center"
                  >
                    <svg
                      width="13"
                      height="14"
          ...
          ResolveFunction={() => {
            if (active) {
              if (ActiveSelectUsers?.length > 0) {
                dispatch(
                  activeAndDeactiveCustomerMaster(ActiveSelectUsers)
                ).then(() => {
                  dispatch(getAllCustomerMaster()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
              ...
              if (InactiveSelectUsers?.length > 0) {
                dispatch(
                  activeAndDeactiveCustomerMaster(InactiveSelectUsers)
                ).then(() => {
                  dispatch(getAllCustomerMaster()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
