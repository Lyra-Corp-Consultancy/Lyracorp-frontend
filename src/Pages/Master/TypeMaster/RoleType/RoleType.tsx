...
function RoleType() {
  ...
  const removeRoleType = () => {
    const res = dispatch(deleteTypeMaster({ values: deleteType, type: "role" }));
    res.then(() => {
      setDelete([]);
      fetchRoleType();
    });
  ...
  return (
    <div className="min-h-screen">
      ...
      {confirmation === "add" && <ConfirmationBox RejectButtonText="Cancel" RejectFunction={() => setConfirmation("")} ResolveButtonText="Save" ResolveFunction={addRoleType} message="Are You sure want to save?" />}

      {confirmation === "edit" && <ConfirmationBox RejectButtonText="No" RejectFunction={() => setConfirmation("")} ResolveButtonText="Yes" ResolveFunction={editRoleType} message="Are You sure want to change?" />}

      {confirmation === "delete" && <DeleteConfirmationBox RejectFunction={() => setConfirmation("")} ResolveFunction={removeRoleType} message="Are you sure want to delete?" />}
    </div>
  );
}
