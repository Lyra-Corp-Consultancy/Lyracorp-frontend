...
function DepartmentType() {
    ...
    const removeDepartmentType = () => {
        const res = dispatch(deleteTypeMaster({ values: deleteType, type: "department" }))
        res.then(() => {
            setDelete([])
            fetchDepartmentType()
        })
    ...
    const addDepartmentType = () => {
        const res = dispatch(postType({ value: input, type: "department" }))
        res.then(() => {
            fetchDepartmentType()
        })
        setConfirmation("")
    ...
    const editDepartmentType = () => {
        const res = dispatch(updateType({ id: deleteType[0], val: input, type: "department" }))
        res.then(() => {
            setDelete([])
            setInput(" ")
            fetchDepartmentType()
    ...
    return (
        <div className='min-h-screen'>
            ...
            <div className='px-10 pt-4'>
                <h1 className='text-[20px] roboto-bold'>Department Type Master</h1>
                <div className='w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]'>
                    <div className='bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg '>
                        <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>Department Type List</h2>
                            <div className='w-full rounded-lg h-[85%] shadow-md shadow-[#00000055]'>
                                ...
                                <div className='flex gap-5 items-center bg-[#5970F5] px-2 py-1'>
                                    ...
                                    <p className='text-white '>Department Type</p>
                                </div>
                       ...
                       {((deleteType?.length===1 && permissions?.edit?.includes("department type")) ||  permissions?.add?.includes("department type")) && <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>{deleteType.length === 1 ? "Edit" : "Add"} Department Type</h2>
            ...
            {confirmation === "add" && <ConfirmationBox RejectButtonText='Cancel' RejectFunction={() => setConfirmation("")} ResolveButtonText='Save' ResolveFunction={addDepartmentType} message='Are You sure want to save?' />}

            {confirmation === "edit" && <ConfirmationBox RejectButtonText='No' RejectFunction={() => setConfirmation("")} ResolveButtonText='Yes' ResolveFunction={editDepartmentType} message='Are You sure want to change?' />}

            {confirmation === "delete" && <DeleteConfirmationBox RejectFunction={() => setConfirmation("")} ResolveFunction={removeDepartmentType} message='Are you sure want to delete?' />}
        </div>
    )
}
