...
function DepartmentType() {
  ...
  const removeDepartmentType = () => {
    const res = dispatch(
      deleteTypeMaster({ values: deleteType, type: "department" })
    );
    res.then(() => {
      setDelete([]);
      fetchDepartmentType();
  ...
  const editDepartmentType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "department" })
    );
    res.then(() => {
      setDelete([]);
      setInput(" ");
    ...
    <div className="min-h-screen">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
            ...
            {((deleteType?.length === 1 &&
              ...
              permissions?.add?.includes("department type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} Department Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
      ...
      {confirmation === "add" && (
        <ConfirmationBox
          ...
          ResolveFunction={addDepartmentType}
      ...
      {confirmation === "edit" && (
        <ConfirmationBox
          ...
          ResolveFunction={editDepartmentType}
      ...
      {confirmation === "delete" && (
        <DeleteConfirmationBox
          RejectFunction={() => setConfirmation("")}
          ResolveFunction={removeDepartmentType}
