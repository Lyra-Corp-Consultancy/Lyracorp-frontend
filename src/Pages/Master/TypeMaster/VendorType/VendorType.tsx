...
function VendorType() {
  ...
  const removeVendorType = () => {
    const res = dispatch(
      deleteTypeMaster({ values: deleteType, type: "vendor" })
    );
    res.then(() => {
      setDelete([]);
      fetchVendorType();
  ...
  const addVendorType = () => {
    ...
    if (!finalword?.includes(converted)) {
      const res = dispatch(postType({ type: "vendor", value: input }));
      res.then(() => {
        fetchVendorType();
      });
      setConfirmation("");
  ...
  const editVendorType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "vendor" })
    );
    res.then(() => {
      setDelete([]);
      setInput(" ");
    ...
    <div className="min-h-screen">
      ...
      <div className="px-10 pt-4">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
            ...
            {((deleteType?.length === 1 &&
              permissions?.edit?.includes("vendor type")) ||
              permissions?.add?.includes("vendor type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} Vendor Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
      ...
      {confirmation === "edit" && (
        <ConfirmationBox
          ...
          ResolveFunction={editVendorType}
      ...
      {confirmation === "delete" && (
        <DeleteConfirmationBox
          RejectFunction={() => setConfirmation("")}
          ResolveFunction={removeVendorType}
