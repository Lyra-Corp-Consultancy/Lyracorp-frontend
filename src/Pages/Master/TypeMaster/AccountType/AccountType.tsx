...
function AccountType() {
  ...
  const removeAccountType = () => {
    const res = dispatch(
      deleteTypeMaster({ values: deleteType, type: "account" })
    );
    res.then(() => {
      setDelete([]);
      fetchAccountType();
  ...
  const editAccountType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "account" })
    );
    res.then(() => {
      setDelete([]);
      setInput(" ");
    ...
    <div className="min-h-screen">
      ...
      <div className="px-10 pt-4">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
            ...
            {((deleteType?.length === 1 &&
              permissions?.edit?.includes("account type")) ||
              permissions?.add?.includes("account type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} Account Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
      ...
      {confirmation === "add" && (
        <ConfirmationBox
          ...
          ResolveFunction={addAccountType}
      ...
      {confirmation === "edit" && (
        <ConfirmationBox
          ...
          ResolveFunction={editAccountType}
      ...
      {confirmation === "delete" && (
        <DeleteConfirmationBox
          RejectFunction={() => setConfirmation("")}
          ResolveFunction={removeAccountType}
