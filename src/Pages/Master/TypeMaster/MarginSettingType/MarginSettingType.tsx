...
function MarginSettingType() {
  ...
  const removeMarginSettingType = () => {
    const res = dispatch(deleteTypeMaster({ values: deleteType, type: "marginSetting" }))
    res.then(() => {
      setDelete([])
      fetchMarginSettingType()
    })
  ...
  const addMarginSettingType = () => {
    const res = dispatch(postType({ value: input, type: "marginSetting" }))
    res.then(() => {
      fetchMarginSettingType()
    })
    setConfirmation("")
  ...
  const editMarginSettingType = () => {
    const res = dispatch(updateType({ id: deleteType[0], val: input, type: "marginSetting" }))
    res.then(() => {
      setDelete([])
      setInput(" ")
      fetchMarginSettingType()
  ...
  return (
    <div className='min-h-screen'>
      ...
      <div className='px-10 pt-4'>
        <h1 className='text-[20px] roboto-bold'>Margin Setting Type Master</h1>
        <div className='w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]'>
          <div className='bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg '>
            <div className='w-1/2 h-full px-5 py-2'>
              <h2 className='text-black font-semibold'>Margin Setting Type List</h2>
              <div className='w-full rounded-lg h-[85%] shadow-md shadow-[#00000055]'>
                ...
                <div className='flex gap-5 items-center bg-[#5970F5] px-2 py-1'>
                  ...
                  <p className='text-white '>Margin Setting Type</p>
                </div>
            ...
            {((deleteType?.length===1 && permissions?.edit?.includes("margin setting type")) ||  permissions?.add?.includes("margin setting type")) &&<div className='w-1/2 h-full px-5 py-2'>
              <h2 className='text-black font-semibold'>{deleteType.length === 1 ? "Edit" : "Add"} Margin Setting Type</h2>
      ...
      {confirmation === "add" && <ConfirmationBox RejectButtonText='Cancel' RejectFunction={() => setConfirmation("")} ResolveButtonText='Save' ResolveFunction={addMarginSettingType} message='Are You sure want to save?' />}

      {confirmation === "edit" && <ConfirmationBox RejectButtonText='No' RejectFunction={() => setConfirmation("")} ResolveButtonText='Yes' ResolveFunction={editMarginSettingType} message='Are You sure want to change?' />}

      {confirmation === "delete" && <DeleteConfirmationBox RejectFunction={() => setConfirmation("")} ResolveFunction={removeMarginSettingType} message='Are you sure want to delete?' />}
    </div>
  )
}
