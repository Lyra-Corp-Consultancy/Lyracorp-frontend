...
function CertificationType() {
  ...
  const permissions = useSelector((state: any) => state.data?.user?.permissions);
    ...
    const removeCertificationType = () => {
        const res = dispatch(deleteTypeMaster({ values: deleteType, type: "certification" }))
        res.then(() => {
            setDelete([])
            fetchCertificationType()
        })
    ...
    const addCertificationType = () => {
        const res = dispatch(postType({ value: input, type: "certification" }))
        res.then(() => {
            fetchCertificationType()
        })
        setConfirmation("")
    ...
    const editCertificationType = () => {
        const res = dispatch(updateType({ id: deleteType[0], val: input, type: "certification" }))
        res.then(() => {
            setDelete([])
            setInput(" ")
            fetchCertificationType()
    ...
    return (
        <div className='min-h-screen'>
            ...
            <div className='px-10 pt-4'>
                <h1 className='text-[20px] roboto-bold'>Certification Type Master</h1>
                <div className='w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]'>
                    <div className='bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg '>
                        <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>Certification Type List</h2>
                            <div className='w-full rounded-lg h-[85%] shadow-md shadow-[#00000055]'>
                                ...
                                <div className='flex gap-5 items-center bg-[#5970F5] px-2 py-1'>
                                    ...
                                    <p className='text-white '>Certification Type</p>
                                </div>
                      ...
                      {((deleteType?.length===1 && permissions?.edit?.includes("certification type")) ||  permissions?.add?.includes("certification type")) &&  <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>{deleteType.length === 1 ? "Edit" : "Add"} Certification Type</h2>
            ...
            {confirmation === "add" && <ConfirmationBox RejectButtonText='Cancel' RejectFunction={() => setConfirmation("")} ResolveButtonText='Save' ResolveFunction={addCertificationType} message='Are You sure want to save?' />}

            {confirmation === "edit" && <ConfirmationBox RejectButtonText='No' RejectFunction={() => setConfirmation("")} ResolveButtonText='Yes' ResolveFunction={editCertificationType} message='Are You sure want to change?' />}

            {confirmation === "delete" && <DeleteConfirmationBox RejectFunction={() => setConfirmation("")} ResolveFunction={removeCertificationType} message='Are you sure want to delete?' />}
        </div>
    )
}
