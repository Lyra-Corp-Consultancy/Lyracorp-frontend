...
function UOMType() {
  ...
  const addUOMType = () => {
    ...
    const convertedDescription = input?.des?.split(" ").join("").toLowerCase();
    const finalword = values?.map((x) => {
      return x?.value?.name?.split(" ").join("").toLowerCase();
    });

    const finalword2 = values?.map((x) => {
      return x?.value?.des?.split(" ").join("").toLowerCase(); //For description
    });
  ...
  const editUOMType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "uom" })
    );
    res.then(() => {
      setDelete([]);
      setInput({ des: "", name: "" });
      ...
      <div className="px-10 pt-4">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
            ...
            {((deleteType?.length === 1 &&
              permissions?.edit?.includes("UOM type")) ||
              permissions?.add?.includes("UOM type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} UOM Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
