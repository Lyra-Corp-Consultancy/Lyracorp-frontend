...
function ShippingType() {
    ...
    const removeShippingType = () => {
        const res = dispatch(deleteTypeMaster({ values: deleteType, type: "shipping" }))
        res.then(() => {
            setDelete([])
            fetchShippingType()
        })
    ...
    const addShippingType = () => {
        const res = dispatch(postType({ value: input, type: "shipping" }))
        res.then(() => {
            fetchShippingType()
        })
        setConfirmation("")
    ...
    const editShippingType = () => {
        const res = dispatch(updateType({ id: deleteType[0], val: input, type: "shipping" }))
        res.then(() => {
            setDelete([])
            setInput(" ")
            fetchShippingType()
    ...
    return (
        <div className='min-h-screen'>
            ...
            <div className='px-10 pt-4'>
                <h1 className='text-[20px] roboto-bold'>Shipping Type Master</h1>
                <div className='w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]'>
                    <div className='bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg '>
                        <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>Shipping Type List</h2>
                            <div className='w-full rounded-lg h-[85%] shadow-md shadow-[#00000055]'>
                                ...
                                <div className='flex gap-5 items-center bg-[#5970F5] px-2 py-1'>
                                    ...
                                    <p className='text-white '>Shipping Type</p>
                                </div>
                       ...
                       {((deleteType?.length===1 && permissions?.edit?.includes("shipping type")) ||  permissions?.add?.includes("shipping type")) && <div className='w-1/2 h-full px-5 py-2'>
                            <h2 className='text-black font-semibold'>{deleteType.length === 1 ? "Edit" : "Add"} Shipping Type</h2>
            ...
            {confirmation === "add" && <ConfirmationBox RejectButtonText='Cancel' RejectFunction={() => setConfirmation("")} ResolveButtonText='Save' ResolveFunction={addShippingType} message='Are You sure want to save?' />}

            {confirmation === "edit" && <ConfirmationBox RejectButtonText='No' RejectFunction={() => setConfirmation("")} ResolveButtonText='Yes' ResolveFunction={editShippingType} message='Are You sure want to change?' />}

            {confirmation === "delete" && <DeleteConfirmationBox RejectFunction={() => setConfirmation("")} ResolveFunction={removeShippingType} message='Are you sure want to delete?' />}
        </div>
    )
}
