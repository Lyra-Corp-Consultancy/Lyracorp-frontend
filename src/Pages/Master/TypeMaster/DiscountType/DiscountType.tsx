...
function DiscountType() {
  ...
  const removeDiscountType = () => {
    const res = dispatch(
      deleteTypeMaster({ values: deleteType, type: "discount" })
    );
    res.then(() => {
      setDelete([]);
      fetchDiscountType();
  ...
  const addDiscountType = () => {
    ...
    if (!finalword?.includes(converted)) {
      const res = dispatch(postType({ type: "discount", value: input }));
      res.then(() => {
        fetchDiscountType();
      });
      setConfirmation("");
  ...
  const editDiscountType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "discount" })
    );
    res.then(() => {
      setDelete([]);
      setInput(" ");
    ...
    <div className="min-h-screen">
      ...
      <div className="px-10 pt-4">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
            ...
            {((deleteType?.length === 1 &&
              permissions?.edit?.includes("discount type")) ||
              permissions?.add?.includes("discount type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} Discount Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
      ...
      {confirmation === "add" && (
        <ConfirmationBox
          ...
          ResolveFunction={addDiscountType}
      ...
      {confirmation === "edit" && (
        <ConfirmationBox
          ...
          ResolveFunction={editDiscountType}
      ...
      {confirmation === "delete" && (
        <DeleteConfirmationBox
          RejectFunction={() => setConfirmation("")}
          ResolveFunction={removeDiscountType}
