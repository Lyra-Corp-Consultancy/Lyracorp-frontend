...
function TransportType() {
  ...
  const removeTransportType = () => {
    const res = dispatch(
      deleteTypeMaster({ values: deleteType, type: "transport" })
    );
    res.then(() => {
      setDelete([]);
      fetchTransportType();
  ...
  const addTransportType = () => {
    ...
    if (!finalword?.includes(converted)) {
      const res = dispatch(postType({ type: "transport", value: input }));
      res.then(() => {
        fetchTransportType();
      });
      setConfirmation("");
  ...
  const editTransportType = () => {
    const res = dispatch(
      updateType({ id: deleteType[0], val: input, type: "transport" })
    );
    res.then(() => {
      setDelete([]);
      setInput(" ");
    ...
    <div className="min-h-screen">
      ...
      <div className="px-10 pt-4">
        ...
        <div className="w-full  justify-between rounded-lg shadow-md shadow-[#00000055] pt-2 px-5 pb-16 bg-[#F1F3FF] h-[60vh]">
          <div className="bg-[#ffffff] w-full flex justify-between  shadow-md shadow-[#00000055] h-full rounded-lg ">
                  ...
                  {search?.map((x: any) => (
                    <div className="flex gap-5 items-center bg-white px-2 py-1">
                      {deleteType.includes(x?._id) ? (
                        ...
                        <div
                          onClick={() => setDelete((prev) => [...prev, x?._id])}
            ...
            {((deleteType?.length === 1 &&
              permissions?.edit?.includes("transport type")) ||
              permissions?.add?.includes("transport type")) && (
              <div className="w-1/2 h-full px-5 py-2">
                <h2 className="text-black font-semibold">
                  {deleteType.length === 1 ? "Edit" : "Add"} Transport Mode Type
                </h2>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
      ...
      {confirmation === "add" && (
        <ConfirmationBox
          ...
          ResolveFunction={addTransportType}
      ...
      {confirmation === "edit" && (
        <ConfirmationBox
          ...
          ResolveFunction={editTransportType}
      ...
      {confirmation === "delete" && (
        <DeleteConfirmationBox
          RejectFunction={() => setConfirmation("")}
          ResolveFunction={removeTransportType}
