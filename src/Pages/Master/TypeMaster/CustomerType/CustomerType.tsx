...
import { deleteCustomerType, getCustomerType, postCustomerType, updateCustomerType } from '../../../../utils/redux/actions'
...
function CustomerType() {
    ...
    const fetchCustomerType = () => {
        const res = dispatch(getCustomerType())
        res.then((res: any) => {
    ...
    const removeCustomerType = () => {
        const res = dispatch(deleteCustomerType(deleteType))
        res.then(() => {
            setDelete([])
            fetchCustomerType()
        })
    ...
    const addCustomerType = () => {
        const res = dispatch(postCustomerType(input))
        res.then(() => {
            fetchCustomerType()
        })
        setConfirmation("")
    ...
    const editCustomerType = () => {
        const res = dispatch(updateCustomerType({ id: deleteType[0], val: input }))
        res.then(() => {
            setDelete([])
            setInput(" ")
            fetchCustomerType()
