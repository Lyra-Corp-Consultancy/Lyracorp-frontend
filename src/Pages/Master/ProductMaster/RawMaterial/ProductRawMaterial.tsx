...
import { activeAndDeactiveProductRawMaterial, getAllProductRawMaterial, getType } from "../../../../utils/redux/actions";
import ActiveUsers from "./ActiveUsers";
import DeactiveUsers from "./DeactiveUsers";
import DeleteConfirmationBox from "../../../../components/DeleteConfirmationBox";

function ProductRawMaterial() {
  ...
  return (
    <div className="h-[83vh] w-screen">
      <div className="w-full px-5 h-[90%] pt-2">
        <h1 className="text-xl roboto-bold">Raw Material Master</h1>
        <div className="bg-[#F1F3FF] shadow-md mt-2 w-full p-4 rounded-lg h-full">
          <div className="flex gap-4">
            <button onClick={() => setActive(true)} className={"flex flex-col  rounded-md px-4 py-2 " + (active ? "bg-[#5970F5] text-white" : "bg-[#C3CBFF] text-black ")}>
              ...
              <svg width="24" height="24" viewBox="0 0 24 24"   fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className=" roboto-regular text-[15px]">Active Raw Material </p>
              </div>
            ...
            <button onClick={() => setActive(false)} className={"flex flex-col  rounded-md px-4 py-2 " + (!active ? "bg-[#5970F5] text-white" : "bg-[#C3CBFF] text-black ")}>
              <div className="flex items-center gap-3">
                ...
                <p className=" roboto-regular text-[15px]">Inactive Raw Material </p>
              </div>
          ...
          <div className="bg-white rounded-lg w-full pt-3 h-[80%] shadow-md mt-4">
            <h2 className="roboto-bold ms-3 text-[20px]">Raw Material List</h2>
            <div className="px-4 flex justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                {permissions?.add?.includes("raw material") && <Link to={"/master/product-master/raw-material/add"} className="bg-[#5970F5] flex px-3 py-2 rounded-md text-white gap-2 items-center">
          ...
          ResolveFunction={() => {
            if (active) {
              if (ActiveSelectUsers?.length > 0) {
                dispatch(activeAndDeactiveProductRawMaterial(ActiveSelectUsers)).then(() => {
                  dispatch(getAllProductRawMaterial()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
                  });
              ...
              if (InactiveSelectUsers?.length > 0) {
                dispatch(activeAndDeactiveProductRawMaterial(InactiveSelectUsers)).then(() => {
                  dispatch(getAllProductRawMaterial()).then((res: any) => {
                    setData(res.payload);
                    setFiltered(res.payload);
                  });
