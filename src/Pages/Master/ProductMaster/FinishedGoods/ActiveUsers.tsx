...
import styles from "./ProductMaster.module.scss";
import DeleteConfirmationBox from "../../../../components/DeleteConfirmationBox";
import { useDispatch, useSelector } from "react-redux";
import { activeAndDeactiveProductFinishedGoods } from "../../../../utils/redux/actions";
...
function ActiveUsers({ data, dropDowns, inActiveCustomer,selected,setSelected }: Prop) {
    ...
    <div className="h-[80%] overflow-auto w-full">
            ...
            <tr className="border relative">
              ...
              <th className="relative ">
                ...
                <div className={"hidden hover:flex flex-col gap-[1px] absolute right-0 z-20 " + styles.option}>
                 {permissions?.delete?.includes("finished goods") &&  <button
                    onClick={() => {
                      setInactive(x?._id);
                    }}
                  ...
                  <button onClick={()=>navigate("/master/product-master/finished-goods/view/"+x?._id)} className="bg-[#E0E4FF] rounded-md shadow-md shadow-[#00000040] gap-2 items-center text-black flex px-2 py-1">
                 ...
                 {permissions?.edit?.includes("finished goods") && <button onClick={()=>navigate("/master/product-master/finished-goods/edit/"+x?._id)} className="bg-[#E0E4FF] rounded-md shadow-md shadow-[#00000040] gap-2 items-center text-black flex px-2 py-1">
      ...
      {inactive && (
        <DeleteConfirmationBox
          ...
          ResolveFunction={() => {
            dispatch(activeAndDeactiveProductFinishedGoods([inactive])).then(() => {
              inActiveCustomer();
              setInactive("")
            });
          }}
          message="Do you want to inactive this Finished Goods?"
          pos="Inactive"
        />
      )}
    </div>
