...
import { editProductFinishedGoods, getAllProductFinishedGoods, getAllProductRawMaterial } from "../../../../utils/redux/actions";
          ...
          <div className="bg-[#F1F3FF] grid grid-cols-2 gap-36 shadow-md mt-2 w-full p-4 rounded-lg h-full">
            <div className="bg-white w-full h-full shadow-md rounded-md px-3 py-2">
              ...
              <div className="shadow-[0px_0px_4px_rgba(0,0,0,0.385)] mt-3 rounded-md w-full flex items-center justify-between px-7 py-2">
                <label className="flex gap-4">
                  FG Material
                  <Select value={selectedProducts?.[0]?.productName || ""}>
                    {finishedGoods.map((x) => (
                      <li
              ...
              {selectedProducts[0]?.mappings && <><table className="w-full">
                <thead>
                ...
                <tbody>
                  {selectedProducts[0]?.mappings?.map((x:any) => (
                    <tr>
                      <td className="border">{rawMaterials.filter((y) => y?._id === x?.product)[0]?.productName}</td>
              ...
              <div className="flex justify-center py-3 items-center gap-4 w-full">
                ...
                <button onClick={()=>{
                  ...
                  setSelectedProducts([...temp]);
                }} type="submit" className="bg-[#5970F5] px-3 py-1 rounded text-white text-[14px]">
                  Edit
                </button>
              </div></>}
            </div>
            <div className="bg-white w-full h-full shadow-md rounded-md px-3 py-2">
              ...
              <div className="shadow-[0px_0px_4px_rgba(0,0,0,0.385)] mt-3 rounded-md w-full flex items-center justify-between px-7 py-2">
                <label className="flex gap-4">
                  FG Material
                  <Select value={selectedProducts?.[1]?.productName || ""}>
                    {" "}
                    {finishedGoods.map((x) => (
                      <li
                        className="px-3 hover:bg-slate-200 py-1 transition-all duration-100"
              ...
              <form
                onSubmit={(e) => {
                  ...
                  dispatch(editProductFinishedGoods({ id: selectedProducts[1]?._id, data: { mappings } })).then(() => {
                    const temp = [...selectedProducts];
                    temp[1] = {};
                    setSelectedProducts([...temp]);
                    setMappings([{}]);
              ...
              >
                ...
                <div className="flex justify-end items-center py-2">
                  <button type="button" className="bg-[#5970F5] px-3 py-1 rounded text-white text-[14px]" onClick={() => setMappings([...mappings, {}])}>
                    + Add More
                  </button>
                </div>
                <div className="flex justify-center items-center gap-4 w-full">
                  ...
                  <button type="submit" className="bg-[#5970F5] px-3 py-1 rounded text-white text-[14px]">
                    Save
                  </button>
                </div>
              </form>
